...
def select_domain(request):
    if request.method == 'POST':
        ...
        if student_type in ['school', 'university']:
            from .models import UserProfile
            ...
            if student_type == 'university':
                ...
                return redirect('home')
    return render(request, 'select_domain.html')

@login_required
def uni_home(request):
    return render(request, 'uni_home.html')

@login_required
def uni_courses(request):
    return render(request, 'uni_courses.html')

@login_required
def uni_books(request):
    return render(request, 'uni_books.html')
